...
PropheseeWrapperPublisher::PropheseeWrapperPublisher() :
    ...
    if (publish_cd_)
        pub_cd_events_ =  rclcpp::Node::create_publisher<ddsmetadata::msg::DDSMetaData>(
    ...
    camera_.add_runtime_error_callback([this](const Metavision::CameraException &e) { RCLCPP_WARN(rclcpp::Node::get_logger(),"%s", e.what()); });

    // Get the sensor config
    Metavision::CameraConfiguration config = camera_.get_camera_configuration();
    auto &geometry                        = camera_.geometry();
    RCLCPP_INFO(rclcpp::Node::get_logger(), "[CONF] Width:%i, Height:%i", geometry.width(), geometry.height());
    RCLCPP_INFO(rclcpp::Node::get_logger(), "[CONF] Serial number: %s", config.serial_number.c_str());
    ...
    try {
        if (raw_file_to_read_.empty()) {
            ...
            if (!biases_file_.empty()) {
                RCLCPP_INFO(rclcpp::Node::get_logger(), "[CONF] Loading bias file: %s", biases_file_.c_str());
                camera_.biases().set_from_file(biases_file_);
            }
        } else {
            camera_ = Metavision::Camera::from_file(raw_file_to_read_);
            RCLCPP_INFO(rclcpp::Node::get_logger(),"[CONF] Reading from raw file: %s", raw_file_to_read_.c_str());
        }

        camera_is_opened = true;
    } catch (Metavision::CameraException &e) { RCLCPP_WARN(rclcpp::Node::get_logger(), "%s", e.what()); }
        ...
        Metavision::CallbackId cd_callback =
            camera_.cd().add_callback([this](const Metavision::EventCD *ev_begin, const Metavision::EventCD *ev_end) {
                ...
                if (ev_begin < ev_end) {
                    ...
                    event_buffer_current_time_ = rclcpp::Time(start_timestamp_.nanoseconds() + (ev_begin->t * 1000.00));
                    ...
                    event_buffer_current_time_= rclcpp::Time(start_timestamp_.nanoseconds() + (ev_end - 1)->t * 1000.00);
                }

                if ((event_buffer_current_time_ - event_buffer_start_time_) >= rclcpp::Duration::from_seconds(100.0e-6)) {
                    /** Create the message **/
                    ddsmetadata::msg::DDSMetaData::UniquePtr dds_buffer(new ddsmetadata::msg::DDSMetaData());
                    char* mdata_ptr = reinterpret_cast<char*>(malloc(9));
                    ...
                    dds_buffer->type = 2;
                    dds_buffer->elsize = 1;
                    dds_buffer->total_size =event_buffer_.size()*9;
                    ...
                    memcpy(dds_info_ptr,&height,4);
                    memcpy(dds_info_ptr+4,&width,4);
                    memcpy(dds_info_ptr+8,&buffer_time,8);
                    dds_buffer->mdata = std::vector<unsigned char>(dds_info_ptr,dds_info_ptr+16);
                    std::reverse(dds_buffer->mdata.begin(), dds_buffer->mdata.end());
                    // Copy the events to the ros buffer format
                    for (const Metavision::EventCD *it = std::addressof(event_buffer_[0]);
                         it != std::addressof(event_buffer_[event_buffer_.size()]); ++it) {
                        int64_t time = start_timestamp_.nanoseconds() + it->t * 1000.00;
                        RCLCPP_INFO(rclcpp::Node::get_logger(),"[CONF] The data [width, height] = [%d,%d]", width, height);
                        RCLCPP_INFO(rclcpp::Node::get_logger(),"[CONF] The data [x,y,p,stamp] = [%d,%d,%d,%ld]", it->x, it->y, it->p, time);
                        memcpy(mdata_ptr,&it->x,2);
                        memcpy(mdata_ptr+2,&it->y,2);
                        memcpy(mdata_ptr+4,&it->p,1);
                        memcpy(mdata_ptr+5,&time,4);
                        dds_buffer->mdata.insert(dds_buffer->mdata.begin()+16+insert_index,mdata_ptr,mdata_ptr+9);
                        insert_index += 9;
                        memset(mdata_ptr,0,9);
                    }
                    RCLCPP_INFO(rclcpp::Node::get_logger(),"===============Already publihsed once==============");
                    free(mdata_ptr);
                    free(dds_info_ptr);
                    // Publish the message
